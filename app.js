...
const BACKUP_OPEN_KEY="bp11_backup_open";
const localBackupSection=$("localBackupSection");
if(localBackupSection){
  const savedBackupState=localStorage.getItem(BACKUP_OPEN_KEY);
  localBackupSection.open=savedBackupState==="1";
  const updateBackupStateLabel=()=>{
    const label=localBackupSection.querySelector(".section-state-label");
    if(label)label.textContent=localBackupSection.open?"點擊收合":"點擊展開";
  };
  updateBackupStateLabel();
  localBackupSection.addEventListener("toggle",()=>{
    localStorage.setItem(BACKUP_OPEN_KEY,localBackupSection.open?"1":"0");
    updateBackupStateLabel();
  });
}
